...
<blockquote><a href="http://www.ruby-doc.org/core/classes/Kernel.html#M005966"><strong>load</strong></a>: Loads and executes the Ruby program in the file <em>filename</em>.
...
So, where's the best place to put the files with your custom code? Well, there's a few folders that are on the Rails load path in the first place: <strong>app, lib, vendor and mock</strong> <strong>paths</strong> (<a href="http://github.com/rails/rails/tree/master/railties/lib/initializer.rb#L607-609">source</a>)</div>
<div class="m-description">If you need to add different folders to the load path, that's more than possible. In your config/environment.rb, add whatever folder you want in the config.load_paths variable. For instance,</div>
<div class="m-description">
<pre lang="ruby">config.load_paths += %W( custom )</pre>
</div>
...
<pre lang="ruby">module CoreExtensions
...
Object.instance_eval { include CoreExtensions }</pre>
So now, you can call
<pre lang="ruby">require_core_ext</pre>
...
<pre lang="ruby">String#slugify</pre>
